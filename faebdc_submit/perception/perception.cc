...
const double too_high_rate = 0.15;	//太高的点比例限制
const int building_total_eps = 5;	//不能有太多太高的点
const double Smooth_eps = 0.15;
const double car_area_least = 3.5;
const double car_area_largest = 20.0;
const double density_least = 15;	//car密度判断
...
void GetUnion(Pic &xu)
{
	...
	for(i=0;i<xu.cc;i++)
	{
		bool yuzu=0,yuzd=0,ydzu=0,ydzd=0;
		for(j=i+1;j<xu.cc;j++)
		{
			if(xu.point[j].x-xu.point[i].x>union_dis)
				break;
			if((xu.point[i].x+union_dis-xu.point[j].x)*(xu.point[i].x+union_dis-xu.point[j].x)
				+(xu.point[i].y-xu.point[j].y)*(xu.point[i].y-xu.point[j].y)
				+(xu.point[i].z-xu.point[j].z)*(xu.point[i].z-xu.point[j].z)<union_dis*union_dis)
			{
				if(xu.point[j].y>xu.point[i].y && xu.point[j].y-xu.point[i].y<0.5*union_dis)
				{
					if(xu.point[j].z>xu.point[i].z && xu.point[j].z-xu.point[i].z<0.5*union_dis)
					{
						yuzu=1;
					}
					if(xu.point[j].z<xu.point[i].z && xu.point[i].z-xu.point[j].z<0.5*union_dis)
					{
						yuzd=1;
					}
				}
				if(xu.point[j].y<xu.point[i].y && xu.point[i].y-xu.point[j].y<0.5*union_dis)
				{
					if(xu.point[j].z>xu.point[i].z && xu.point[j].z-xu.point[i].z<0.5*union_dis)
					{
						ydzu=1;
...
void DetectSingleObject(vector<Vector> &now,interface::perception::PerceptionObstacles &perception_result,Vector trans)
{
	...
	for(int j=now.size()-1;j>=0;j--)
	{
		...
		vx+=block_shift;
		vy+=block_shift;
		height=min(height,sval[vx][vy]);
		if(now[j].z-sval[vx][vy]>too_high_eps)
			too_high_num++;
	}

	double ss=(xb-xa)*(yb-ya);
	...
	for(int i=1;i<=100;i++)
	{
		...
		while(a<1 || a>1000000000)
			a=rand();
		double angle=2.0*atan(1.0);
		angle*=a;
		angle/=1e9;
		double txs=sin(angle);
		double tys=cos(angle);
		/*if(rand()%2)
			txs=-txs;*/
		double txa,txb,tya,tyb;
		txa=tya=inf;
		txb=tyb=-inf;
		for(int j=now.size()-1;j>=0;j--)
		{
			txa=min(txa,now[j].x*tys-now[j].y*txs);
			txb=max(txb,now[j].x*tys-now[j].y*txs);
			tya=min(tya,now[j].y*tys+now[j].x*txs);
			tyb=max(tyb,now[j].y*tys+now[j].x*txs);
		}

		double ssnow=(txb-txa)*(tyb-tya);
		if(ssnow<ss)
		{
			ss=ssnow;
			xs=txs;
	...
	if(now.size()/ss < density_least )
		return;
	if(car_area_largest<ss || ss<car_area_least)
		return;
...

	{
	    ...
	    obstacle->set_type(interface::perception::ObjectType::CAR);
	    {
	      ...
	      double x0=xa;
	      double y0=ya;
	      polygon_point->set_x((ys*x0+xs*y0)+trans.x);
	      polygon_point->set_y((-xs*x0+ys*y0)+trans.y);
	      polygon_point->set_z(height);
	    }
	    {
	      auto* polygon_point = obstacle->add_polygon_point();
	      double x0=xa;
