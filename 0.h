...
const double faebdc_INF = 1e8;
const double faebdc_eps = 1e-7;
...
struct faebdc_Node {
  ...
  faebdc_Node() {}
  template <typename T>
  faebdc_Node(const T& p) {
    x = p.x();
    y = p.y();
  }
};

double faebdc_dist(const faebdc_Node& a, const faebdc_Node& b) {
  double dx = a.x - b.x;
  double dy = a.y - b.y;
  return dx * dx + dy * dy;
}
...
class faebdc_Graph {
 public:
  faebdc_Graph() {}
  int add_node(const faebdc_Node& n) {
    faebdc_Nodes.push_back(n);
    G.push_back(std::vector<int>());
    return faebdc_Nodes.size() - 1;
  }
  void add_edge(int a, int b) { G[a].push_back(b); }

  std::vector<faebdc_Node> shortest_path(faebdc_Node start, faebdc_Node end) {
    int sid = -1, eid = -1;
    for (int i = 0; i < faebdc_Nodes.size(); i++) {
      if (sid == -1 || faebdc_dist(faebdc_Nodes[i], start) < faebdc_dist(faebdc_Nodes[sid], start)) {
        sid = i;
      }
      if (eid == -1 || faebdc_dist(faebdc_Nodes[i], end) < faebdc_dist(faebdc_Nodes[eid], end)) {
        eid = i;
      }
    }
    ...
    std::vector<int> fa(faebdc_Nodes.size(), -1);
    std::vector<double> d(faebdc_Nodes.size(), faebdc_INF);
    ...
    while (q.size()) {
      ...
      if (c > d[u] + faebdc_eps) continue;

      for (int v : G[u]) {
        double nc = c + faebdc_dist(faebdc_Nodes[u], faebdc_Nodes[v]);
        if (nc + faebdc_eps < d[v]) {
          d[v] = nc;
          fa[v] = u;
    ...
    CHECK(fa[eid] != -1);
    std::vector<faebdc_Node> path;
    for (int u = eid; u != sid; u = fa[u]) {
      path.push_back(faebdc_Nodes[u]);
    }
    path.push_back(faebdc_Nodes[sid]);
 ...
 private:
  std::vector<faebdc_Node> faebdc_Nodes;
  std::vector<std::vector<int> > G;
};

interface::geometry::Point3D faebdc_get_start(const interface::map::Lane& lane) {
  CHECK(lane.central_line().point_size() >= 1);
  return lane.central_line().point(0);
}

interface::geometry::Point3D faebdc_get_end(const interface::map::Lane& lane) {
  CHECK(lane.central_line().point_size() >= 1);
  return lane.central_line().point(lane.central_line().point_size() - 1);
}

bool faebdc_equal_Point3D(const interface::geometry::Point3D& a, const interface::geometry::Point3D& b) {
  // cout << std::abs(a.x() - b.x()) << " " <<  std::abs(a.y() - b.y()) << " " << std::abs(a.z() -
  // b.z()) << endl;
  return std::abs(a.x() - b.x()) < faebdc_eps && std::abs(a.y() - b.y()) < faebdc_eps &&
         std::abs(a.z() - b.z()) < faebdc_eps;
}

void faebdc_process_map(interface::map::Map& map_proto) {
  for (int i = 0; i < map_proto.lane_size(); i++) {
    auto* l = map_proto.mutable_lane(i);

    auto start = faebdc_get_start(*l);
    auto end = faebdc_get_end(*l);

    // find predecessor
    for (int j = 0; j < map_proto.lane_size(); j++) {
      if (i == j) continue;
      auto n_start = faebdc_get_start(map_proto.lane(j));
      auto n_end = faebdc_get_end(map_proto.lane(j));

      // successor
      if (faebdc_equal_Point3D(end, n_start)) {
      ...
      // predeceesor
      if (faebdc_equal_Point3D(start, n_end)) {
...
faebdc_Graph faebdc_load_graph(interface::map::Map& map_proto) {
  faebdc_Graph g;
  pnc::map::MapLib maplib;
  map_proto = maplib.map_proto();
  faebdc_process_map(map_proto);
  ...
  for (const auto& lane : map_proto.lane()) {
    ...
    for (const auto& p : lane.central_line().point()) {
      int cur_id = g.add_node(faebdc_Node(p));
      if (last_id >= 0)
...
// and kefaebdc_eps running with the speed over 5m/s until reaches destination.





class faebdcVehicleAgent : public simulation::VehicleAgent {
 public:
  explicit faebdcVehicleAgent(const std::string& name) : VehicleAgent(name) {}

  void Initialize(const interface::agent::AgentStatus&  agent_status ) /*override*/ 
  {
    // Nothing to initialize
    g = faebdc_load_graph(map_proto);
  ...
  interface::control::ControlCommand RunOneIteration(
      const interface::agent::AgentStatus& agent_status) override {
      	...
      	double STOP_DIS = 19;
        double STOP_EPS = 0.5;
      	double TRAFFIC_DIS = 17;
      	double TRAFFIC_STOP_EPS = 5;
      	double MAX_V = 14;
      	double RATE = 0.226667 * 1.2;
        double S_RATE = 0.35;
      	double PATH_NEXT_POINT_DIS = 6;
      	double CAR_DANGER_DIS = 20;
      	double PERSON_DANGER_DIS = 20;
      	double CAR_LINE_DIS = 6;
      	double PERSON_LINE_DIS = 5;
      	double DANGER_EPS = 2;
      	double DANGER_LINE_DIS = 5;
      	double HALFCYCLE = 23;
...
/*******New Dest*********/      	
	    ...
	    while (path_index < path.size()) {
	      if (CalcDistance(vtp(agent_status.vehicle_status().position()), to_point3d(path[path_index])) <
    ...
    lastp=nowp;
    interface::geometry::Vector3d aim = Jian(dest,vtp(agent_status.vehicle_status().position()));
...
/******* reach dest ***********/
    ...
    if (CalcDistance(vtp(agent_status.vehicle_status().position()),  agent_status.route_status().destination()) <
        (velocity/(MAX_V)) * STOP_DIS + STOP_EPS ) {  // 15 is picked according to table.txt
    ...
    for(const auto &tra : trc.single_traffic_light_status())
    {
    	...
    	if(tra.id().id()==trd.id().id())
    	{
    		...
    		for(const auto &trb : trd.stop_line().point())
    		{
    			...
    			if(last.x()>-1e8)
    			{
            if( CalcDistance(MidPoint( trb , last ),MidPoint(vtp(agent_status.vehicle_status().position()) , nowp ))<3.0)
              must_stop_=true;
				    if (CalcDistance(nowp, MidPoint( trb , last )) <
				        ...
				        {
				        	if((std::abs(aim.x())>=12*std::abs(aim.y()) || std::abs(aim.y())>=12*std::abs(aim.x())) 
				        		 && LineUp(aim,Jian(MidPoint( trb , last), vtp(agent_status.vehicle_status().position()) )) )
				        	{
                    minratio = std::min(minratio, 0.600 * (CalcDistance(nowp, MidPoint( trb , last ))-TRAFFIC_STOP_EPS)/TRAFFIC_DIS) ;
...
/******* reach object ***********/
	...
	for(const auto &ob : agent_status.perception_status().obstacle())
	{
		if(ob.type()==interface::perception::CAR)
		{
			...
			{
				if (CalcDistance(nowp, p) <	 1.000 * CAR_DANGER_DIS)
         	if ( is_turn || LineDis( aim , Jian(p,nowp ) ) < CAR_LINE_DIS  )
				    if( is_turn || LineRight( aim,	 Jian(p,nowp ) )  )
              if( LineUp( aim, Jian(p,nowp ) ) || 
                ( is_turn && LineUp( aim, Jian(p,MidPoint(vtp(agent_status.vehicle_status().position()) , nowp ) ) ) )  )
           		{
		...
		if(ob.type()==interface::perception::PEDESTRIAN || ob.type()==interface::perception::CYCLIST)
		{
			for(const auto &p : ob.polygon_point())
			{
				if (CalcDistance(nowp, p) <		 1.000 * PERSON_DANGER_DIS)
         	if ( is_turn || LineDis( aim, Jian(p,nowp )) < PERSON_LINE_DIS  )
            if( LineUp( aim, Jian(p,nowp ) ) || 
              ( is_turn && LineUp( aim, Jian(p,MidPoint(vtp(agent_status.vehicle_status().position()) , nowp ) ) ) )  )
         		{
         			//position_reached_object_=true;
...
/******* too large velocity ***********/
    ...
    if (velocity > 2 && velocity > 1.100 * MAX_V)
    ...
    if(must_stop_){
      command.set_brake_ratio(1.000 * S_RATE);
    }
	  else if(velocity_too_large_)
      command.set_brake_ratio(std::min(1.000, (velocity/MAX_V-0.1) * S_RATE));  // 0.226667 is picked according to table.txt
    else if(almost_reached_object_)
      command.set_brake_ratio(1.000 * S_RATE);  // 0.226667 is picked according to table.txt
    else if(position_reached_red_light_)
      command.set_brake_ratio(1.000 * S_RATE);  // 0.226667 is picked according to table.txt
    else if (position_reached_destination_) {
      command.set_brake_ratio(1.000 * S_RATE);  // 0.226667 is picked according to table.txt
    }
	  else {
      if (!velocity_reached_threshold_) {
        command.set_throttle_ratio(sqrt(minratio) * RATE);  // 0.226667 is picked according to table.txt
      }
    }
 ...
 private:
  interface::geometry::Point3D to_point3d(faebdc_Node n) {
  ...
  interface::geometry::Point3D vehicle_position(const interface::agent::AgentStatus& agent_status)
  {
    ...
    if(std::abs(resx.x())+std::abs(resx.y())+std::abs(resx.z())<0.001)
    {
      if(lastp.x()>-1e5)
        return lastp;
      else
  ...
  interface::geometry::Point3D lastp;
